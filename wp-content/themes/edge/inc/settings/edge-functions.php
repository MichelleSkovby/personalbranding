		...
		if ( !empty( $edge_list_page ) && $edge_total_page_no > 0 ) {
			...
			$image_attributes = wp_get_attachment_image_src($attachment_id,'edge_slider_image');
				...
				$remove_link = $edge_settings['edge_slider_link'];
					if($remove_link == 0){
						if ($title_attribute != '') {
							$edge_page_sliders_display .= '<h2 class="slider-title"><a href="'.esc_url(get_permalink()).'" title="'.the_title('', '', false).'" rel="bookmark">'.get_the_title().'</a></h2><!-- .slider-title -->';
						}
					}else{
					...
					if($edge_settings['edge_slider_button'] == 0){
						if($excerpt_text == '' || $excerpt_text == 'Read More') :
							$edge_page_sliders_display 	.= '<a title='.'"'.get_the_title(). '"'. ' '.'href="'.esc_url(get_permalink()).'"'.' class="btn-default">'.__('Read More', 'edge').'</a>';
						else:
						$edge_page_sliders_display 	.= '<a title='.'"'.get_the_title(). '"'. ' '.'href="'.esc_url(get_permalink()).'"'.' class="btn-default">'.esc_attr($edge_settings[ 'edge_tag_text' ]).'</a>';
						endif;
							}
			...
			$edge_page_sliders_display .= '</div>	  <!-- end .layer-slider -->
					<a class="slider-prev" id="prev2" href="#"><i class="fa fa-angle-left"></i></a> <a class="slider-next" id="next2" href="#"><i class="fa fa-angle-right"></i></a>
  					<nav class="slider-button" role="navigation" aria-label="'.esc_attr__('Slider Menu','edge').'"> </nav> <!-- end .slider-button -->
				</div> <!-- end .main-slider -->';
		}
...
function edge_scripts() {
	...
	wp_enqueue_script('edge-skip-link-focus-fix', get_template_directory_uri().'/js/skip-link-focus-fix.js', array('jquery'), false, true);
	// Load the html5 shiv.
